...
- **设置入口固定在侧边栏底部**：一行「⚙ 设置」条目（icon + 文字），与列表主体之间用 hairline 或留白分隔；hover 有背景反馈，点击有按下态。不再从编辑器内入口为主。
- 底部区域可同时容纳同步/字数等状态，但保持一行、安静。

### 5.2 设置页
- 布局参考 Notion Settings：左侧分类列表 + 右侧内容（当前的分类结构保留），但视觉全部按 §2-§4 重做。
- 分组标题 12px tertiary 大写/加粗，行高统一，行 label 左、控件右。
- **软件更新**：
  - **不暴露更新地址**：移除「更新地址」输入框（URL 作为内部默认配置/隐藏在高级入口，普通界面不展示）。
  - 只保留一个「检查更新」按钮；点击后按钮内出现 loading 态（spinner 替换文字）。
...
- [ ] app.dart：新 ThemeData token（§2/§3），删除 CupertinoTheme 依赖
- [ ] glass.dart：废弃毛玻璃，改实色表面组件
- [ ] settings_view.dart：全部 Cupertino 控件替换；更新区按 §5.2 重做；加 toast
- [ ] sidebar.dart：底部设置入口（§5.1）
- [ ] editor.dart：光标参数（§5.3）；文字按钮 → icon+tooltip
- [ ] 全局圆角审计：只允许 4/6/8
