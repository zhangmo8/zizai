...
分类仅在功能已接线时显示：`备份` 需要 `BackupManager`；`关于` 常驻（版本信息与软件更新组随 `UpdateChecker` 接线显隐，帮助组始终可用）。备份入口默认打开「备份」；状态栏今日进度点击 → 打开「写作设置」并聚焦每日目标字数。
...
| 目录 | 章节排序 | 正序（旧章在上，默认）/ 倒序（新章在上）；章节很多时倒序浏览最新章节更方便 |
...
`docsOrder.<id>`（asc/desc）、`volume.<id>.enabled/.chapters/.mode`、
`volume.<id>.name.<n>`（settings 表）。
...
| 备份 / 本地备份 | 导出备份文件 | 把全部数据（含分卷/章节状态/备注）存为一份本地 `.json` 快照；桌面走保存对话框，Android 分享文件 |
| 备份 / 本地备份 | 从备份文件恢复 | 选 `.json` 快照 → 二次确认 → 全量替换（恢复前自动 `.bak` 滚动保留 3 份）；与 R2 备份同一格式、可互通 |
...
| 数据 / 导入 | 从橙瓜码字导入 | 选择橙瓜码字数据库（`<uid>.db`），把其「图书→卷→章节」映射为「笔记本→分卷→章节」追加导入；解析在后台 isolate 进行，完成后 toast 导入数量 |
...
| 导出备份文件 | 备份 | 本地快照导出；失败显示错误（不可写路径） |
| 从备份文件恢复 | 备份 | 选文件 → 二次确认 → 全量替换；恢复前自动 `.bak`；恢复后刷新书架与设置 |
| 从橙瓜码字导入 | 数据 | 选 `.db` → 后台解析 → 批量入库 → 刷新书架；失败显示可读错误 |
...
- **本地恢复/橙瓜导入中**：按钮 busy 态，完成 toast 显示数量，失败 toast 显示可读错误。
