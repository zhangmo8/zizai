...
| 38 | v2.2.0 真机反馈 | Android 新建/删除章节后 Drawer 白屏（存量 bug，长期潜伏）：移动端拖拽包装层 ReorderableDelayedDragStartListener 缺 key，release 无 assert、列表 diff 错位崩；桌面路径 key 正常故未暴露。已修 + 移动端复现用例（debugIsDesktopPlatformOverride 注入）防复发；遗留：CI 移动端分支覆盖靠 override 注入，真机冒烟仍靠人工（2026-08-28） | 已完成 |
